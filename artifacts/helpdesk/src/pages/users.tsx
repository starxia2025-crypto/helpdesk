    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Gestiona accesos y roles del sistema.</p>
        </div>
        ...
        <Table>
          <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
            <TableRow>
              ...
              <TableHead className="text-right font-semibold">Último acceso</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
                ...
                <TableRow key={user.id} className="group">
                  ...
                  {currentUser?.role === 'superadmin' && (
                    <TableCell className="text-slate-600 dark:text-slate-400 text-sm">
                      {user.tenantName || <span className="text-slate-400 italic">Sistema</span>}
                    </TableCell>
                  )}
                  <TableCell>
        ...
        {usersData && usersData.totalPages > 1 && (
          <div className="p-4 border-t flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" disabled={page === usersData.totalPages} onClick={() => setPage(p => p + 1)}>Siguiente</Button>
            </div>
          </div>
        )}
      </div>
